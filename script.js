/* global Vue */
const GyrateHitApp = Vue.createApp({
	data() {
		return {
			arc: [180, 270],
			taps: 0,
...
if ("ontouchstart" in window) {
	window.addEventListener("touchstart", GyrateHitApp.tap);
} else {
	window.addEventListener("mousedown", GyrateHitApp.tap);
	window.onkeypress = (e) => {
		if (e.keyCode == 32) {
			if (GyrateHitApp.state === "stopped") {
				GyrateHitApp.startPlay();
			} else {
				GyrateHitApp.tap(e);
			}
		}
